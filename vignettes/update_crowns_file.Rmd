...
new_shp_corrected$id[!new_shp_corrected$id %in% gil$id]
gil$id[!gil$id %in% new_shp_corrected$id] # ajouter l'id 984 depuis dbx


sf <- new_shp_corrected %>% filter(id %in% gil$id)

sf <- new_shp_corrected %>% arrange(id)
gil <- gil %>% arrange(id)
...
sf <- sf %>% select(-c(idmodif,samegeom))
